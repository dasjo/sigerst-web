---
title: Zwinglipasshütte (SAC, Mehrtagestour)
kategorie: wandern
dauer:
  von: 5     # Hüttenaufstieg hin & zurück
  bis: 16    # als 2-Tages-Tour mit Übernachtung
  zustieg: 2.5
schwierigkeit:
  skala: SAC-T
  wert: T2–T3
  einfach: mittel
saison:
  monate: [6, 7, 8, 9, 10]
  hinweis: "Sommer–Herbst bewartet; Winterraum ganzjährig (Code). Reservation Pflicht"
gpx: /assets/gpx/zwinglipasshuette.gpx
gpx_quelle: BRouter (OSM-Wanderwegnetz), grob aufgelöst – Verlauf prüfen
start:
  name: Lisighaus, Wildhaus (via Gamplüt, familiengeeignet) oder via Mutschensattel (T3)
  koordinaten: [47.2013, 9.342]  # swisstopo, verifiziert
distanz_km: null
hoehenmeter: null
familienfreundlich: true
anreise: "Ab Haus zu Fuß oder mit Gamplüt-Gondel starten"
seite: /wandern-ausflugsziele/#berghütten-für-mehrtagestouren
links:
  - titel: SAC Toggenburg – Clubhütte
    url: https://sac-toggenburg.ch/clubhuette/
  - titel: Reservation
    url: https://www.alpsonline.org/reservation/calendar?hut_id=30&lang=de_CH
---

SAC-Hütte am Zwinglipass (42 Schlafplätze) mit Aussicht bis zum Mönch. Basis für Altmann, Klettergarten Zwinglipass und Churfirsten-Touren. Hüttenschlafsack und Bargeld mitbringen.
