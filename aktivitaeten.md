---
layout: default
title: Aktivitäten
description: Skifahren, Winter, Wandern, Mountainbike und Kulinarik rund um Wildhaus
---

# Aktivitäten

Rund um das Ferienhaus Sigerst gibt es das ganze Jahr viel zu erleben. Hier die Übersicht – Details auf den einzelnen Seiten.

**[→ Alle Touren auf der Karte ansehen]({{ site.baseurl }}/karte/)** – mit Filter nach Dauer, Schwierigkeit und Saison.

<ul class="aktivitaeten-list">
  <li>
    <a href="{{ site.baseurl }}/skifahren/">Skifahren</a>
    <p>Vom Haus zur Talstation: Bushaltestelle «Bären», Skibus zum Zentrum und ins Skigebiet Wildhaus–Alt St. Johann.</p>
  </li>
  <li>
    <a href="{{ site.baseurl }}/winteraktivitaeten/">Winteraktivitäten</a>
    <p>Winterwandern, Schlitteln, Langlauf, Eislaufen – Wege, Schlittelbahnen (z. B. Gamplüt) und Loipen in der Region.</p>
  </li>
  <li>
    <a href="{{ site.baseurl }}/wandern-ausflugsziele/">Wandern &amp; Ausflüge</a>
    <p>Gamplüt, Gulmen, Schafberg, Thurwasserfälle, Klettergarten Stoss, Klangweg, Schönenbodensee und weitere Touren im Toggenburg.</p>
  </li>
  <li>
    <a href="{{ site.baseurl }}/mountainbike/">Mountainbike</a>
    <p>Trails und Rundtouren ab Oberdorfbahn, Bike-Transport und Verleih in der Nähe.</p>
  </li>
  <li>
    <a href="{{ site.baseurl }}/kulinarik/">Kulinarik</a>
    <p>Toggenburger Spezialitäten, Restaurants, Berggasthöfe und regionale Produkte.</p>
  </li>
</ul>

<p><a href="{{ site.baseurl }}/">← Zurück zur Startseite</a></p>
